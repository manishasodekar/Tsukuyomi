...
kafka_service = KafkaService(group_id="clinical_ner")
kafka_client = kafka_service.create_clients(group_id="clinical_ner")
                ...
                for consumer in kafka_service.post_consumer:
                    if consumer.value.decode('utf-8') != '':
                        if consumer.topic == heconstants.EXECUTOR_TOPIC:
                            ...
                            if message_dict.get("state") == "ClinicalNer" and not message_dict.get("completed"):
                                aipreds = aiPreds()
                                if message_dict.get("req_type") == "encounter":
                                    stream_key = message_dict.get("care_req_id")
                                    file_path = message_dict.get("file_path")
                                    logger.info(f"Starting CLINICAL_NER :: {stream_key} :: {file_path}")
                                else:
                                    request_id = message_dict.get("request_id")
                                    file_path = message_dict.get("file_path")
                                    logger.info(f"Starting CLINICAL_NER for platform:: {request_id} :: {file_path}")
